"""
Sanity-check the Sixers puzzles by computing the valid-answer pool for every row
...
Run after build_sixers.py finishes:  python3 scripts/validate_sixers.py
...
PLAYERS_PATH = os.path.join(ROOT, 'data', 'sixers', 'players.json')
PUZZLES_PATH = os.path.join(ROOT, 'data', 'sixers', 'puzzles.json')
...
if not os.path.exists(PLAYERS_PATH):
    print(f"ERROR: {PLAYERS_PATH} not found. Run `python3 scripts/build_sixers.py` first.")
    sys.exit(1)
...
if issues_found == 0:
    ...
    print("or an over-tight constraint combination.")
